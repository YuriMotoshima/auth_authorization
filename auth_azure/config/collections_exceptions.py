...
class collections_exceptions(Exception):
    ...
    def traceback_cause(self):
        try:
            if self.traceback_exception.stack:
                logging.error(json.dumps({frame[0]: {
                    "error": self.str_error, "path": frame[1].filename,
                    "line": frame[1].lineno, "code": frame[1]._line} for frame
                    in enumerate(self.frame_summary)},
                    indent=2))
