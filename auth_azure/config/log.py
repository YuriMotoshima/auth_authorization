...
def loginit(name_file_log: str = "GPC", dev_env: str = "DEV", disable_log: str | bool = True):
    ...
    """
    basicConfig(level=10, format=formatter, handlers=[
    ...
    if valid_variables:
        ...
        if filename_regex.match(name_file_log):
            filename = f"{
                name_file_log} - {datetime.now().strftime('%d-%m-%Y %H')}.log"

        new_filename = f'[DEV] {
            filename}' if dev_env == 'DEV' else f'[PROD] {filename}'
        ...
        basicConfig(level=10, format=formatter, handlers=[
                    file_handler, stdout_handler], encoding='utf-8')
